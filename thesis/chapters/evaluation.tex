...
Benchmark results for the runs we conducted can be found in Figure~\ref{fig:evaluation:stamp}.
...
We already discussed in detail the performance of the \emph{labyrinth} application for Ohua and STM that is shown in Fig.~\ref{fig:evaluation:labyrinth} in Chapter~\ref{sec:preliminary}.
...
The Software Transactional Memory implementation itself performs just as the original STAMP implementation by Minh et al.~\cite{minh2008stamp}, reinforcing the validity of our comparison.\todo{which comparison?????}
...
But as was the case with STM, the intruder+ version performs slightly worse than the smaller input set.\todo{why?}
...
In \emph{genome}, the curve shapes of our STAMP results from Fig.~\ref{fig:evaluation:stamp} and the Rust STM in Fig.~\ref{fig:evaluation:genome} results are similar in the way that they exhibit peak performance for a medium amount of threads before it declines.
...
But due to the large portions of still sequential code, i.e., the nucleotide sequence deduplication, no real gains can be made in terms of execution time.\todo{Clones als mögliche Ursache?}
